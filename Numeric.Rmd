...
Regression models indicate that age and social media reliance are key predictors of misinformation beliefs, with older individuals and those less reliant on Facebook holding lower beliefs.
</span>

![](numeric.jpg){width=100%}
...
## Descriptive Statistics Analysis & Visualisation
...
The ages in our sample range from 16 to 75, with a mean age of 44.57 and a median age of 44, indicating a nearly symmetrical distribution. The standard deviation (16.43) suggests a moderate spread of ages around the mean. 

A skewness of 0.02 confirms the distribution is almost symmetric, while a kurtosis of 1.77 suggests a platykurtic shape — a flatter distribution with fewer extreme outliers than a normal distribution. A density plot shows that individuals in their 20s and 50s are the most represented age groups.
...
### Reliance on Facebook for Covid info

```{r visualise facebook, echo=FALSE}
...
#### Age & Gender's Associations with Reliance on Facebook

**Gender vs Reliance on Facebook**
...
A Chi-square test of independence with Monte Carlo simulation (2000 replicates) was conducted to examine the relationship between gender and knowledge of Facebook. The test produced a χ²(NA) = 2.70, p = 0.841. The standard Chi-square approximation was not valid due to small expected frequencies, so a simulated p-value was used instead. 

The high p-value (p > 0.05) suggests that there is no statistically significant association between gender and knowledge of Facebook. The observed differences in counts are likely due to random variation rather than a meaningful relationship.

**Age vs Reliance on Facebook**
...
``` {r visualise beliefs, echo=FALSE}
...
A proportion distribution of belief scores shows 308 people (61.6%) rejected all five statements, 116 people (23.2%) rejected most but endorsed one, 38 people (7.6%) endorsed two statements, 18 people (3.6%) endorsed three, 12 people (2.4%) endorsed four, and 8 people (1.6%) endorsed all five statements. 

#### Associations of Misinformation Beliefs with Age, Gender, Reliance on FB

**Gender vs Misinformation Beliefs**

``` {r gender vs beliefs, echo=FALSE}
...
**Reliance on FB vs Misinformation Beliefs**
...
Reliance on Facebook for coronavirus information is positively correlated with belief scores (rho = 0.23, p < 0.001), meaning those who rely more on Facebook tend to believe more misinformation statements. This correlation is also weak but statistically significant. 

The boxplot above shows a high concentration of 0 misinformation beliefs for those who do not get much information from Facebook, and no such concentration of values for those who do get information from Facebook. The line graph indicates a positive trend, with mean misinformation beliefs rising as Facebook reliance increases.

-------------------------------------------------------------------------------------------------------------

## Linear Regression to Predict Misinformation Beliefs
...
```{r age-model, echo=FALSE, message=FALSE, warning=FALSE}
my_model %>%
  lm(
    beliefs ~ age,
    data = .
...
```{r gender-model, echo=FALSE, message=FALSE, warning=FALSE}
my_model %>%
  lm(
    beliefs ~ Female,
    data = .
...
```{r facebook-model, echo=FALSE, message=FALSE, warning=FALSE}
my_model %>%
  lm(
...
```{r full-model, echo=FALSE, message=FALSE, warning=FALSE}
full_model <- my_model %>%
  lm(
